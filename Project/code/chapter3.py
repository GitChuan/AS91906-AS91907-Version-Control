...
class MathBattleGame3:
    ...
    def generate_question(self):
        ...
        # All question types
        question_type = random.choice(["square_root", "multiple", "digit_sum"])
    ...
    def draw_hero_info(self):
        ...
        self.screen.blit(small_font.render(f"HP: {self.player_hp}/{self.player_max_hp}", True, (255, 100, 100)),
                         (padding + 160, padding + 60))
        self.screen.blit(small_font.render(f"Attack: {self.player_attack}", True, (255, 255, 255)),
                         (padding + 160, padding + 90))
        self.screen.blit(small_font.render(f"Intelligence: {self.player_intelligence}", True, (100, 255, 255)),
                         (padding + 160, padding + 120))
        self.screen.blit(small_font.render(f"Charisma: {self.player_charisma}", True, (255, 255, 100)),
                         (padding + 160, padding + 150))
    ...
    def draw_dragon_info(self):
        ...
        # Set enemy's detail
        small_font = pygame.font.SysFont("arial", 24)
        self.screen.blit(small_font.render(f"HP: {self.dragon_hp}/{self.dragon_max_hp}", True, (255, 100, 100)),
                         (rect.x + 160, rect.y + 70))
        self.screen.blit(small_font.render(f"Attack: {self.dragon_attack}", True, (255, 255, 255)),
                         (rect.x + 160, rect.y + 110))
    ...
    def run(self):
        ...
        while running:
            ...
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    ...
                    if self.game_over:
                        ...
                        user_ans = self.user_input.strip().lower()  # Turn user's input into lower case and delete blank space
                        correct = False  # Boolean type for checking correct
                        isValid = True  # Boolean type for checking valid
                        if self.answer in ["yes", "no"]:  # Multiple question
                            correct = (user_ans == self.answer)
                        else:  # This will cover the rest question and check all questions valid when reach here
                            try:
                                correct = abs(float(user_ans) - float(self.answer)) < 0.01
                            except:
                                self.feedback = "Input is invalid!"
                                isValid = False
                        # If the question is correct and valid (no need for check valid as valid is always true if correct is true)
                        if correct:
                            # Check whether a critical hit has been made.
                            is_crit = self.calculate_crit()
                            damage = self.player_attack * (
                                2 if is_crit else 1)  # Double the attack amount if it is a critical hit.
                            ...
                            self.add_damage_text(f"-{damage}", (dmg_x, dmg_y),
                                                 (255, 60, 60) if is_crit else (
                                                 255, 200, 50))  # Add into the damage list

                        elif isValid:  # Wrong answer
                            # Update this question into mistake book.
                            update_mistake_book1234(self.username, "chapter3", self.question, self.user_input,
                                                    self.answer)
                            ...
                            if self.calculate_dodge():
                                ...
                                dmg_x = 10 + 160
                                dmg_y = 60
                                self.add_damage_text(str(self.dragon_attack), (dmg_x, dmg_y),
                                                     (60, 255, 60))  # Add into the damage list
            ...
            if not self.game_over and elapsed > self.time_limit:
                # Update this question into mistake book
                update_mistake_book1234(self.username, "chapter3", self.question, "", self.answer)
                # Check whether a dodge has been made.
                if self.calculate_dodge():
                    self.dodge_flash_time = pygame.time.get_ticks()
